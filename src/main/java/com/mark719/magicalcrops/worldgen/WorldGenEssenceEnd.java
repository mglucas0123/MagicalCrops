...
public class WorldGenEssenceEnd implements IWorldGenerator
{
    ...
    private void generateEnd(final World world, final Random random, final int i, final int j) {
        if (ConfigMain.essOreEnd) {
            for (int k = 0; k < 25; ++k) {
                final int chunkX = i + random.nextInt(16);
                final int chunkY = random.nextInt(256);
                final int chunkZ = j + random.nextInt(16);
                new WorldGenMineableEnd(MBlocks.MinicioOreEnd, ConfigMain.maxVeinSizeEnd).generate(world, random, chunkX, chunkY, chunkZ);
            }
        }
    }
